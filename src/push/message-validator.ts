...
function validateWebPushConfig(webPushConfig: WebPushConfig, notification?: Notification) {
  if (
    !validator.isNonEmptyString(webPushConfig.notification.title) &&
    !validator.isNonEmptyString(notification.title)
  ) {
    throw new Error("title must not be empty");
  }
  if (
    !validator.isNonEmptyString(webPushConfig.notification.body) &&
    !validator.isNonEmptyString(notification.body)
  ) {
    throw new Error("body must not be empty");
  }
  if (
    validator.isNonEmptyString(webPushConfig.notification.dir) &&
  ...
  if (
    validator.isNonEmptyString(webPushConfig.headers.urgency) &&
...
function validateAndroidNotification(
  ...
  if (
    !validator.isNonEmptyString(androidNotification.title) &&
    !validator.isNonEmptyString(notification.title)
  ) {
    throw new Error("title must not be empty");
  }
  ...
  if (
    !validator.isNonEmptyString(androidNotification.body) &&
    !validator.isNonEmptyString(notification.body)
  ) {
    throw new Error("body must not be empty");
  }
  ...
  if (
    validator.isNonEmptyString(androidNotification.group) &&
    validator.isNonEmptyString(androidNotification.notify_id)
  ) {
    throw new Error("notify_id must be empty when group exist");
  }

  if (
    validator.isNonEmptyString(androidNotification.importance) &&
...
function validateClickAction(clickAction: ClickAction) {
  ...
  switch (clickAction.type) {
    case TypeIntent:
      if (
        !validator.isNonEmptyString(clickAction.intent) &&
        !validator.isNonEmptyString(clickAction.action)
      ) {
...
function validateLightSetting(lightSettings: LightSettings) {
  ...
  if (
    validator.isNonEmptyString(lightSettings.light_off_duration) &&
    !Pattern.exec(lightSettings.light_off_duration)
  ) {
    throw new Error("Wrong input format");
  }
  if (
    validator.isNonEmptyString(lightSettings.light_on_duration) &&
    !Pattern.exec(lightSettings.light_on_duration)
  ) {
    throw new Error("Wrong input format");
  }
}
